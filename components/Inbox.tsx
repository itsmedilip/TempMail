...
const InboxItem: React.FC<{ message: Message; onClick: () => void }> = ({ message, onClick }) => (
    <li
        onClick={onClick}
        className={`p-4 transition-colors duration-200 cursor-pointer border-b border-gray-700 hover:bg-[#39404a]`}
    >
        <div className="flex items-start justify-between">
            <p className={`text-sm truncate pr-2 ${!message.seen ? 'font-bold text-white' : 'text-gray-300'}`}>
                {escapeHTML(message.from.name || message.from.address)}
            </p>
            <span className="text-xs text-gray-400 flex-shrink-0">{formatDate(message.createdAt)}</span>
        </div>
        <div>
            <p className={`text-sm truncate mt-1 ${!message.seen ? 'text-gray-200' : 'text-gray-400'}`}>{escapeHTML(message.subject)}</p>
            <p className="text-xs text-gray-400 truncate mt-1">{escapeHTML(message.intro)}</p>
        </div>
    </li>
);
